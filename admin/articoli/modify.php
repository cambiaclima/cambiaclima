<?php
  ...
  $total_pages = ceil($total_rows / $no_of_records_per_page);
	...
	$sql = "SELECT * FROM articolo LIMIT $offset, $no_of_records_per_page";
	$result = $conn->query($sql);
	$currentPage = 'Modifica articoli';
?>
<html lang="it">
<head>
	...
	<body>
		...
		<main class="container-md shadow-sm bg-white pt-3" role="main">
			<form action="modify_articolo.php" method="post">
			  <fieldset class="form-group">
					  ...
					  <thead class="thead-dark">
						...
						if ($result->num_rows > 0) {
							...
							while($row = $result->fetch_assoc()) {
								print '<tr>
									<td>
										<input type="radio" name="Radios" id="Radios'.($i+$offset).'" value="'.$row["IdArt"].'" checked>
									</th>
				...
				<div class="row">
					<div class="col">
						<label for="invia_uno">Seleziona l'Articolo da modificare</label>
					</div>
					<div class="col-2">
