<?php
  ...
  $total_pages_sql = "SELECT COUNT(*) FROM approfondimento";
  $result = mysqli_query($conn,$total_pages_sql);
  $total_rows = mysqli_fetch_array($result)[0];
  $total_pages = ceil($total_rows / $no_of_records_per_page);

	$sql = "SELECT * FROM approfondimento LIMIT $offset, $no_of_records_per_page";
		...
		<main class="container-md shadow-sm bg-white pt-3" role="main">
			<form action="modifica.php" method="post">
			  <fieldset class="form-group">
					  ...
					  <thead class="thead-dark">
						...
						if ($result->num_rows > 0) {
							...
							while($row = $result->fetch_assoc()) {
								print '<tr>
									<td>
										<input type="radio" name="Radios" id="Radios'.($i+$offset).'" value="'.$row["IdApprofondimento"].'" checked>
									</th>
				...
				<div class="row">
					<div class="col">
						<label for="invia_uno">Seleziona l'approfondimento da modificare</label>
					</div>
					<div class="col-2">
						<input class="btn btn-primary" type="submit"  name="invia_uno" id="invia_uno" value="Modifica"/>
					</div>
				</div>
			</form>
