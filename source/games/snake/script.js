...
window.addEventListener("DOMContentLoaded", function (event) {
  ...
  function getNextPosition() {
    ...
    switch (snakeDirection) {
      case "right": {
        ...
        if (nextPosition % width == 0) throw Error("Il serpente ha sbattuto sul muro, ora la C02 non verra' piu ' riciclata e tutta la flora collassera'");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("Il serpente si e' morso da solo, ora la C02 non verra' piu ' riciclata e tutta la flora collassera'");
        return nextPosition;
      }
      case "left": {
        const nextPosition = headPosition - 1;
        if (nextPosition % width == width - 1 || nextPosition < 0)
          throw Error("Il serpente ha sbattuto sul muro, ora la C02 non verra' piu ' riciclata e tutta la flora collassera'");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("Il serpente si e' morso da solo, ora la C02 non verra' piu ' riciclata e tutta la flora collassera'");
        return nextPosition;
      ...
      case "up": {
        const nextPosition = headPosition - width;
        if (nextPosition < 0) throw Error("Il serpente ha sbattuto sul muro, ora la C02 non verra' piu ' riciclata e tutta la flora collassera'");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("Il serpente si e' morso da solo, ora la C02 non verra' piu ' riciclata e tutta la flora collassera'");
        return nextPosition;
  ...
  function getDirection(first, second) {
    ...
    throw Error("le due tessere non sono collegate");
  }
